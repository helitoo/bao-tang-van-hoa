...
export const getDriveId = (url: string) => {
  ...
  const match = str.match(/\/file\/d\/(.+?)\/(view|preview|edit|copy)/) || str.match(/id=(.+?)(&|$)/);
...
const ArtifactCard: React.FC<ArtifactCardProps> = ({ item, compact = false }) => {
  ...
  return (
    ...
    >
      <div className="overflow-hidden relative bg-stone-100 dark:bg-stone-800 flex items-center justify-center aspect-square">
        ...
        {badges.length > 0 && (
          <div className="absolute top-2 left-2 flex flex-col gap-1 z-10 pointer-events-none">
            {badges.map((badge) => (
              <span key={badge?.id} className={`text-[8px] font-extrabold uppercase tracking-tighter px-1.5 py-0.5 rounded-sm shadow-sm backdrop-blur-md border ${badge?.id === 'bvqg' ? 'bg-amber-500/90 text-white border-amber-400' : 'bg-viet-red/90 text-white border-red-400'}`}>
                {badge?.name}
              </span>
            ))}
          </div>
      ...
      <div className="p-3 flex-1 border-t border-stone-100 dark:border-stone-800 flex flex-col">
        <h4 className={`${compact ? 'text-xs' : 'text-sm'} font-bold group-hover:text-viet-red dark:text-stone-100 dark:group-hover:text-viet-red transition-colors mb-1 leading-tight line-clamp-2 uppercase`}>
          {item.name}
        </h4>
