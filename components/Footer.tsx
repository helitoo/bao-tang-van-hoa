...
const Footer: React.FC<FooterProps> = ({ isSidebar = false }) => {
  ...
  return (
    <footer
      className={`bg-stone-100 dark:bg-stone-900 border-t border-stone-200 dark:border-stone-800 transition-colors duration-300 ${isSidebar ? "py-8 px-6" : "py-12 px-4 sm:px-6 lg:px-8"}`}
    >
      <div
        className={
          isSidebar
            ? "space-y-8"
            : "max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12"
        }
      >
        ...
        <div className="space-y-4">
          ...
          <ul className="space-y-2 text-sm font-medium">
            ...
            <li>
              <button className="text-stone-500 dark:text-stone-400 hover:text-viet-red transition-colors text-left">
                {t("nav_contribute")}
              </button>
            </li>
          </ul>
