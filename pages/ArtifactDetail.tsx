...
const ArtifactDetail: React.FC<ArtifactDetailProps> = ({ artifacts }) => {
  ...
  if (!item) return <div className="py-32 text-center text-stone-400 font-medium">Not Found</div>;
      ...
      >
        <div className="p-8 space-y-10 flex-grow">
          <div className="space-y-4">
            <h1 className="text-3xl md:text-4xl font-extrabold text-stone-900 dark:text-stone-100 tracking-tight leading-tight">{item.name}</h1>
            <div className="flex flex-wrap gap-2">
              {(item.categories || []).map(catId => {
                const name = getCategoryName(catId);
                return name && (
                  <span key={catId} className="text-[9px] font-extrabold bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400 px-2 py-1 border border-stone-200 dark:border-stone-700">
                    {name}
                  </span>
                );
              })}
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-4 text-[13px] bg-stone-50 dark:bg-stone-800/40 p-5 border border-stone-100 dark:border-stone-800">
            <div className="space-y-1">
              ...
              <span className="font-extrabold text-stone-800 dark:text-stone-200 leading-tight block">{renderOrPlaceholder(item.artifact_date)}</span>
            </div>
            <div className="space-y-1">
              <span className="block text-stone-400 dark:text-stone-500 font-extralight uppercase text-[9px] tracking-widest">
                {t('artifact_location')}
              ...
              <span className="font-extrabold text-stone-800 dark:text-stone-200 leading-tight block">{renderOrPlaceholder(item.location)}</span>
            </div>
            <div className="space-y-1">
              <span className="block text-stone-400 dark:text-stone-500 font-extralight uppercase text-[9px] tracking-widest">
                {t('artifact_author')}
              ...
              <span className="font-extrabold text-stone-800 dark:text-stone-200 leading-tight block">{renderOrPlaceholder(item.author)}</span>
            </div>
            <div className="space-y-1">
              <span className="block text-stone-400 dark:text-stone-500 font-extralight uppercase text-[9px] tracking-widest">
              ...
              <span className="font-extrabold text-stone-800 dark:text-stone-200 leading-tight block">{renderOrPlaceholder(item.contributor)}</span>
            </div>
            <div className="col-span-2 pt-3 border-t border-stone-200 dark:border-stone-700 mt-1">
              <span className="block text-stone-400 dark:text-stone-500 font-extralight uppercase text-[9px] tracking-widest mb-1">{t('artifact_public_date')}</span>
          ...
          <div className="space-y-5">
            ...
            {(item.supporting_images || []).length > 0 ? (
              <div className="space-y-3">
                {(item.supporting_images || []).map((url, idx) => <ReferenceItem key={idx} url={url} />)}
              </div>
            ) : <p className="text-xs italic text-stone-400 font-medium">{t('placeholder_none')}</p>}
          </div>

          <div className="space-y-6 pt-6 border-t border-stone-100 dark:border-stone-800">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-extrabold text-stone-900 dark:text-stone-100 italic uppercase">{t('artifact_suggested')}</h3>
              <Link to="/search" className="text-[10px] font-extrabold uppercase tracking-widest text-viet-red hover:underline">{t('see_all')}</Link>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {similarArtifacts.map((sim) => <ArtifactCard key={sim.id} item={sim} compact />)}
            </div>
          </div>
        </div>
        <Footer isSidebar />
