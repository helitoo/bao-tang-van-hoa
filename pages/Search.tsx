...
const Search: React.FC<SearchProps> = ({ artifacts = [] }) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    if (selectedCats.length > 0) {
      ...
      results = results.filter(artifact => {
        if (!artifact || !artifact.categories || !Array.isArray(artifact.categories)) return false;
        return Object.values(selectedByGroup).every(groupSelections => {
          return groupSelections.some(selectedId => artifact.categories.includes(selectedId));
        });
      });
    }
    ...
    if (lowerQuery) {
      results = results
        .filter(artifact => {
          ...
          const catNames = (artifact.categories || []).map(catId => {
            for (const group of CATEGORY_GROUPS) {
  ...
  const getPageNumbers = () => {
    ...
    else if (end > totalPages) { end = totalPages; start = totalPages - 2; }
    const p: (number | string)[] = [];
    if (start > 1) p.push('...');
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 lg:py-12 flex flex-col lg:flex-row gap-8">
      ...
      <aside className={`w-full lg:w-72 flex-shrink-0 ${showMobileFilters ? 'block' : 'hidden lg:block'}`}>
        <div className="bg-white dark:bg-stone-900 p-6 border border-stone-200 dark:border-stone-800 shadow-sm lg:sticky lg:top-24 transition-colors duration-300">
          <div className="flex justify-between items-center mb-6 border-b-2 border-viet-red pb-2">
            <h3 className="text-xl font-extrabold dark:text-stone-100 uppercase">{t('filter_title')}</h3>
            <button onClick={clearAllFilters} className="text-[10px] font-extrabold uppercase tracking-widest text-stone-400 hover:text-viet-red transition-colors">
              {t('filter_clear')}
            </button>
          </div>
          ...
          <form onSubmit={handleSearchSubmit} className="space-y-8">
            <div>
              <label className="block text-sm font-extrabold text-stone-700 dark:text-stone-300 mb-2 uppercase tracking-wide">{t('filter_keyword')}</label>
              <input
                type="text"
                value={filterText}
            ...
            <div className="space-y-6">
              <label className="block text-sm font-extrabold text-stone-700 dark:text-stone-300 uppercase tracking-wide">{t('filter_cats')}</label>
              {CATEGORY_GROUPS.map(group => (
                <div key={group.id} className="space-y-2">
                  <h4 className="text-[10px] font-extralight text-stone-400 dark:text-stone-500 uppercase tracking-widest border-b border-stone-100 dark:border-stone-800 pb-1">{group.title[locale]}</h4>
                  <div className="grid grid-cols-1 gap-1 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                    {group.options.map(opt => (
                      <label key={opt.id} className="flex items-center space-x-2 cursor-pointer group py-0.5">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className={`text-xs transition-colors font-medium ${selectedCats.includes(opt.id) ? 'text-viet-red font-extrabold' : 'text-stone-600 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-stone-100'}`}>
                          {opt.name[locale]}
                        </span>
                      </label>
                    ))}
            ...
            <button type="submit" className="w-full bg-stone-900 dark:bg-stone-700 text-white py-3 hover:bg-viet-red dark:hover:bg-viet-red transition-all transform active:scale-95 font-extrabold uppercase tracking-widest text-[10px] shadow-md">
              {t('filter_btn')}
            </button>
          </form>
        </div>
      ...
      <div className="flex-1">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 border-b-2 border-stone-200 dark:border-stone-800 pb-4 gap-4">
          <div>
            <h2 className="text-3xl font-extrabold dark:text-stone-100 uppercase">
              {filterText.trim() ? `${t('results_for')} "${filterText.trim()}"` : t('nav_search')}
            </h2>
          </div>
          <span className="text-stone-500 font-medium italic text-sm">{t('found')} {filteredResults.length} {t('artifacts')}</span>
        </div>
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center items-center space-x-1">
            ...
            {getPageNumbers().map((p, idx) => {
              if (p === '...') {
                return <span key={`ellipsis-${idx}`} className="px-2 text-stone-400 dark:text-stone-600 font-extrabold select-none text-[10px]">...</span>;
              }
