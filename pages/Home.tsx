...
const StatBarChart = ({ title, data, total, className = "" }: { title: string, data: { label: string, count: number }[], total: number, className?: string }) => {
  return (
    <div className={`bg-white dark:bg-stone-800/50 p-3 md:p-5 border border-stone-200 dark:border-stone-800 shadow-sm flex flex-col transition-colors duration-300 ${className}`}>
            ...
            <div key={idx} className="space-y-1">
              ...
              <div className="w-full h-0.5 bg-stone-100 dark:bg-stone-900 overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.max(percentage, item.count > 0 ? 1 : 0)}%` }}
                ></div>
              </div>
            </div>
          );
    ...
    <div className="space-y-16">
      <section className="relative h-[500px] overflow-hidden bg-stone-900 flex items-center justify-center">
        <div className="absolute inset-0">
           <img 
            src="https://cdnphoto.dantri.com.vn/7VRQ4BCIz8lhoWLWDmuEpIOimm8=/2022/02/19/1-1645206566382.jpeg" 
            alt="Khuê văn các" 
            className="w-full h-full object-cover opacity-60"
            onError={(e) => {
              (e.target as HTMLImageElement).src = "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Noon_Gate_-_Imperial_City_-_Hue_-_Vietnam.jpg/1200px-Noon_Gate_-_Imperial_City_-_Hue_-_Vietnam.jpg";
            }}
           />
           <div className="absolute inset-0 bg-gradient-to-b from-stone-900/70 via-stone-900/40 to-stone-900/80"></div>
        </div>
        <div className="relative z-10 text-center px-4 max-w-4xl">
          <h2 className="text-white text-5xl md:text-7xl font-extrabold mb-6 drop-shadow-2xl leading-tight">
            {t('hero_title')} <br/> <span className="text-white drop-shadow-lg font-medium">{t('hero_subtitle')}</span>
          </h2>
      ...
      <section className="max-w-5xl mx-auto px-4 text-center relative">
        <div className="mb-12 flex flex-col items-center">
            <h1 className="text-stone-900 dark:text-stone-100 text-4xl md:text-5xl font-extrabold mb-2 tracking-tight uppercase">{t('museum_name')}</h1>
            <span className="text-stone-400 dark:text-stone-500 text-xs font-extralight tracking-[0.5em] uppercase opacity-70">{t('museum_sub')}</span>
        </div>
        ...
        <div className="relative p-10 border-2 border-stone-200 dark:border-stone-800 rounded-sm bg-white dark:bg-stone-900 shadow-inner mb-16 overflow-hidden transition-colors duration-300">
          ...
          <div className="relative z-10">
            <div className="flex justify-center mb-6">
                <svg className="w-10 h-10 text-viet-red opacity-80" fill="currentColor" viewBox="0 0 24 24">
            ...
            <p className="text-viet-red font-extrabold uppercase text-xs tracking-widest opacity-90">{t('mission_author')}</p>
      ...
      <section className="max-w-7xl mx-auto px-2 md:px-4">
        <div className="flex flex-col items-center mb-8">
           <h3 className="text-2xl font-extrabold text-stone-800 dark:text-stone-200 mb-2 uppercase">{t('stats_title')}</h3>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 grid-rows-3 md:grid-rows-2 gap-2 md:gap-4 h-[650px] md:h-[500px]">
              ...
              <div className="col-span-2 md:col-span-1 md:row-span-2 bg-white dark:bg-stone-800/50 p-4 border border-stone-200 dark:border-stone-800 flex flex-col items-center justify-center text-center shadow-sm relative overflow-hidden group">
                ...
                <span className="text-[8px] md:text-[10px] font-extralight uppercase tracking-[0.2em] text-stone-400 mb-2">{t('stats_total')}</span>
                <span className="text-4xl md:text-6xl font-extrabold text-viet-red drop-shadow-sm tabular-nums">{stats.total}</span>
              </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1 space-y-6">
              <div className="flex items-center space-x-4">
                ...
                <h3 className="text-3xl md:text-4xl font-extrabold tracking-tight uppercase">{t('ai_search_title')}</h3>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-24">
        <div className="flex items-center justify-between mb-8 border-b-2 border-stone-200 dark:border-stone-800 pb-4">
          <h3 className="text-3xl font-extrabold text-stone-900 dark:text-stone-100 uppercase">{t('featured_title')}</h3>
          <Link to="/search" className="text-xs font-extrabold uppercase tracking-widest text-viet-red hover:underline transition-all">{t('see_all')}</Link>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {artifacts && artifacts.length > 0 ? (
            ...
            <div className="col-span-full py-12 text-center text-stone-400 font-medium">Đang cập nhật hiện vật...</div>
          )}
        </div>
      </section>
